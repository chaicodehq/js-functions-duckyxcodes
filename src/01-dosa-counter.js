...
export function calculateDosaOrder(type, quantity = 1, isSpicy = false) {
  // Your code here
  if (typeof type !== "string" || !Number.isInteger(quantity) || quantity <= 0)
    return null;
  ...
  if (!(type.trim().toLowerCase() in prices)) return null;

  const pricePerDosa = prices[type.trim().toLowerCase()] + (isSpicy ? 10 : 0);
